...
def main():
    ...
    # Carrega a textura difusa do cão (ajuste o caminho!)
    dog_tex  = load_texture("OBJS/Dog/Dog_diffuse.jpg")
    # Carregar objeto
    scene2 = Wavefront('OBJS/Dog/Dog.obj', collect_faces=True, parse=True) # Carrega a geometria do cão

    # Carrega a textura difusa da tartaruga (ajuste o caminho!)
    tartaruga_tex  = load_texture("OBJS/Tartaruga/Tartaruga_diffuse.jpg")
    # Carregar objeto
    scene3 = Wavefront('OBJS/Tartaruga/Tartaruga.obj', collect_faces=True, parse=True) # Carrega a geometria da tartaruga

    # Carrega a textura difusa da pato (ajuste o caminho!)
    pato_tex  = load_texture("OBJS/Pato/Pato_diffuse.jpg")
    # Carregar objeto
    scene4 = Wavefront('OBJS/Pato/Pato.obj', collect_faces=True, parse=True) # Carrega a geometria do pato
    ...
    while running:
        ...
        glScalef(0.5, 0.5, 0.5) # Encolhe o modelo (geralmente OBJs são gigantes)
        ...
        # DESENHO DO MODELO OBJ (CÃO)
        glPushMatrix() # Salva a posição atual
        glTranslatef(1, .5, 0) # Mantém no centro (ou mude aqui para posicionar)
        glRotatef(0, 0, 1, 0) # Corrige a frente do cão
        glRotatef(-90, 1, 0, 0) # Deixa o cão em pé (ajuste comum em modelos OBJ)
        glScalef(0.04, 0.04, 0.04) # Encolhe o modelo (geralmente OBJs são gigantes)
        draw_obj_model(scene2, dog_tex) # Desenha o cão com a textura dele
        glPopMatrix() # Restaura a matriz

        # DESENHO DO MODELO OBJ (TARTARUGA)
        glPushMatrix() # Salva a posição atual
        glTranslatef(-1, 0, 0) # Mantém no centro (ou mude aqui para posicionar)
        glRotatef(72, 0, 1, 0) # Corrige a frente do tartaruga
        glRotatef(-90, 1, 0, 0) # Deixa o tartaruga em pé (ajuste comum em modelos OBJ)
        glScalef(0.04, 0.04, 0.04) # Encolhe o modelo (geralmente OBJs são gigantes)
        draw_obj_model(scene3, tartaruga_tex) # Desenha o cão com a textura dele
        glPopMatrix() # Restaura a matriz

        # DESENHO DO MODELO OBJ (PATO)
        glPushMatrix() # Salva a posição atual
        glTranslatef(-2, 0, 0) # Mantém no centro (ou mude aqui para posicionar)
        glRotatef(-135, 0, 1, 0) # Corrige a frente do pato
        glRotatef(-90, 1, 0, 0) # Deixa o pato em pé (ajuste comum em modelos OBJ)
        glScalef(0.01, 0.01, 0.01) # Encolhe o modelo (geralmente OBJs são gigantes)
        draw_obj_model(scene4, pato_tex) # Desenha o pato com a textura dele
        glPopMatrix() # Restaura a matriz
