...
def inicializaObjetos2():
    ...
    Vao2 = glGenVertexArrays(1) #gera 1 único VAO
    ...
    #adicionar mais três cores

    points2 = [
        #X    Y    Z
		-0.5, 0.75, 0.0, #cima
		-0.25, 0.25, 0.0, #direita
		-0.75, 0.25, 0.0, #esquerda
    ]
    ...
    glBufferData(GL_ARRAY_BUFFER, points2, GL_STATIC_DRAW) #copia os dados do python para dentro do VBO
    ...
    cores2 = np.array(cores2, dtype=np.float32) #converte o array para numpy
    cvbo = glGenBuffers(1) #gera o vbo para as cores
    glBindBuffer(GL_ARRAY_BUFFER, cvbo) # define o VBO das cores como buffer atual
    glBufferData(GL_ARRAY_BUFFER, cores2, GL_STATIC_DRAW) #copia os dados para a memória de vídeo
...
def inicializaObjetos3():
	...
	# Geramos o VAO, definindo um identificador para ele através glGenVertexArrays
    Vao3 = glGenVertexArrays(1) #gera 1 único VAO
    # Damos um bind no VAO, setando ele como VAO atual e colocando o mesmo no topo da máquina de estados do OpenGL
    ...
    points3 = [
        #X    Y    Z
		0.25, 0.75, 0.0, #cima
		0.75, 0.25, 0.0, #direita
		0.25, 0.25, 0.0, #esquerda

        0.75, 0.25, 0.0, #cima
		0.75, 0.75, 0.0, #direita
		0.25, 0.75, 0.0, #esquerda
    ]
    ...
    glBindBuffer(GL_ARRAY_BUFFER, pvbo) #coloca o pvbo no topo da pilha/maquina de estados
    glBufferData(GL_ARRAY_BUFFER, points3, GL_STATIC_DRAW) #copia os dados do python para dentro do VBO
    # Ativamos o primeiro atributo do VAO (índice 0), que é o atributo referente ao buffer das posições dos vértices.
    glEnableVertexAttribArray(0)
    # E então definimos o layout do buffer de vértices:
	# - o primeiro parâmetro (0) significa que estamos definido o layout do atributo 0 (buffer de vértices)
    ...
    cores3 = np.array(cores3, dtype=np.float32) #converte o array para numpy
    cvbo = glGenBuffers(1) #gera o vbo para as cores
    glBindBuffer(GL_ARRAY_BUFFER, cvbo) # define o VBO das cores como buffer atual
    glBufferData(GL_ARRAY_BUFFER, cores3, GL_STATIC_DRAW) #copia os dados para a memória de vídeo
    glEnableVertexAttribArray(1) #ativa o índice 1 para o vbo das cores
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, None) #configura o vbo das cores
...
def inicializaObjetos():
    ...
    points = [
        #X    Y    Z
		-0.5, -0.25, 0.0, #cima
		-0.25, -0.75, 0.0, #direita
		-0.75, -0.75, 0.0, #esquerda

        0.25, -0.25, 0.0, #cima
		0.50, -0.75, 0.0, #direita
		0.0, -0.75, 0.0, #esquerda

        -0.12, -0.75, 0.0, #cima
		0.12, -0.25, 0.0, #direita
		-0.37, -0.25, 0.0 #esquerda
    ]
...
def inicializaRenderizacao():
    global Window, Shader_programm, Vao, WIDTH, HEIGHT, Vao2, Vao3
    ...
    while not glfw.window_should_close(Window):
        ...
        glDrawArrays(GL_TRIANGLES, 0, 9) #a partir do primeiro vértice, desenha 3 vértices
...
def main():
    ...
    inicializaObjetos2() #modelagem dos objetos e envio dos mesmos para a placa de vídeo   
    inicializaObjetos3() #modelagem dos objetos e envio dos mesmos para a placa de vídeo   
